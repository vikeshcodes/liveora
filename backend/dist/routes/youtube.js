...
exports.createYouTubeRouter = void 0;
const express_1 = require("express");
const env_1 = require("../config/env");
const youtube_oauth_1 = require("../integrations/youtube/youtube.oauth");
const asyncHandler = (handler) => (req, res, next) => handler(req, res, next).catch(next);
const adminRedirectUrl = (auth, message) => {
    const url = new URL("/admin/youtube", env_1.env.ADMIN_URL);
...
const createYouTubeRouter = (youtubeService, io) => {
    const router = (0, express_1.Router)();
    router.get("/auth/start", asyncHandler(async (req, res) => {
        const overlayId = String(req.query.overlayId ?? env_1.env.YOUTUBE_DEFAULT_OVERLAY_ID);
        const includeMembership = req.query.includeMembership === "true";
        res.redirect((0, youtube_oauth_1.getYouTubeAuthUrl)(overlayId, includeMembership));
    }));
    router.get("/auth/callback", asyncHandler(async (req, res) => {
        ...
        try {
            const { overlayId } = (0, youtube_oauth_1.consumeOAuthState)(state);
            const tokens = await (0, youtube_oauth_1.exchangeCodeForTokens)(code);
    ...
    router.post("/auth/disconnect", asyncHandler(async (req, res) => {
        const overlayId = String(req.body.overlayId ?? env_1.env.YOUTUBE_DEFAULT_OVERLAY_ID);
