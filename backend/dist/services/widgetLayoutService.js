...
exports.WidgetLayoutService = void 0;
...
const numberOr = (value, fallback, min = 0, max = 4000) => {
    const parsed = Number(value);
    if (!Number.isFinite(parsed))
        return fallback;
    return Math.max(min, Math.min(max, Math.round(parsed)));
};
const rowToLayout = (row) => ({
...
class WidgetLayoutService {
    ...
    kvStore;
    memory = new Map();
    constructor(pool, overlayStore, kvStore = null) {
        this.pool = pool;
        this.overlayStore = overlayStore;
        this.kvStore = kvStore;
    }
    async getLayout(overlayId) {
    ...
    async patchWidget(overlayId, widgetId, patch) {
        ...
        const next = this.normalize(overlayId, { ...(existing ?? {}), ...patch, widgetId }, existing);
        await this.saveAll(overlayId, [next]);
        return this.getLayout(overlayId);
    }
    async resetLayout(overlayId) {
        const overlay = this.overlayStore.getOverlay(overlayId);
        if (!overlay) {
            throw Object.assign(new Error("Overlay not found"), { statusCode: 404 });
        }
    ...
    merge(defaults, saved) {
        ...
        return WIDGET_IDS.map((id) => byId.get(id)).filter(Boolean);
    }
    normalize(overlayId, input, fallback) {
        const item = (input ?? {});
    ...
    async saveAll(overlayId, widgets) {
        ...
        try {
            for (const widget of widgets) {
                await this.pool.query(`INSERT INTO widget_layouts
...
exports.WidgetLayoutService = WidgetLayoutService;
