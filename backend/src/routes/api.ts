...
import { roomForOverlay } from "../realtime/socket";
...
import { DEFAULT_WIDGETS } from "../services/defaults";
...
const sampleForType = (type: EventType, overlayId: string) => {
...
const nowPayload = (payload: Record<string, unknown> = {}) => ({
  ...payload,
...
export const createApiRouter = ({
  ...
  const publicConfigPayload = () =>
    runtimeSettingsService.getPublicSettings(store.getThemes(), publicWidgets(), store.getScenes());
  ...
  router.get(
    ...
    res.json({
      ok: true,
      service: env.LIVEORA_MODE === "docker" ? "LiveOra by Vikesh Codes" : "Vikesh Codes Creator OS backend",
      version: env.VERSION,
      mode: env.LIVEORA_MODE,
      databaseProvider: env.DATABASE_PROVIDER,
      uptimeSeconds: Math.round(process.uptime()),
      socketClients: io.engine.clientsCount,
      databaseConfigured: Boolean(env.DATABASE_URL) || Boolean(kvStore),
      databaseConnected,
      timestamp: new Date().toISOString()
    });
    })
  ...
  router.get("/version", (_req, res) => {
    res.json({
      name: env.LIVEORA_MODE === "docker" ? "LiveOra by Vikesh Codes" : "Vikesh Codes Creator OS",
      version: env.VERSION
    });
  });
  ...
  router.put("/overlays/:id/config", (req, res) => {
    ...
    io.to(roomForOverlay(req.params.id)).emit("overlay:config", config);
  ...
  router.put(
    ...
    asyncHandler(async (req, res) => {
      ...
      io.to(roomForOverlay(req.params.id)).emit("overlay:layout", layout);
  ...
  router.post("/events/test", (req, res) => {
    ...
    const type = String(req.body.type ?? "subscriber") as EventType;
  ...
  router.delete("/events/clear/:overlayId", (req, res) => {
    ...
    io.to("admins").emit(
      "events:cleared",
      nowPayload({ overlayId: req.params.overlayId, cleared, message: "Event history cleared" })
    );
  ...
  router.post("/scenes/:sceneId/activate", (req, res) => {
    ...
    io.to(roomForOverlay(scene.overlayId)).emit("overlay:config", config);
  ...
  router.put("/goals/:overlayId", (req, res) => {
    ...
    io.to(roomForOverlay(req.params.overlayId)).emit("overlay:goal", goal);
  ...
  router.post("/timer/:overlayId/start", (req, res) => {
    ...
    io.to(roomForOverlay(req.params.overlayId)).emit("overlay:timer", state);
  ...
  router.use("/youtube", createYouTubeRouter(youtubeService, io));
